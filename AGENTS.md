...
- `sah` is a Go CLI that authenticates against `https://sah.borca.ai`, prefers an OAuth 2.0 Device Authorization Grant token set stored locally, still accepts an already-stored contributor API key, and uses the native Episteme 3 `/api/v3/*` client, assignment, account, and recognition endpoints.
